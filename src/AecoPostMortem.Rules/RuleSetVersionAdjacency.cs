...
public static class RuleSetVersionAdjacency
{
    ...
    /// here by each version's own <see cref="RuleSetVersion.FirstSessionStartedAt"/> — and returns
    /// the two full <see cref="RuleSetVersion"/> values (identity, window and session count) so a
    /// caller never has to look either back up by hash.
    /// </summary>
