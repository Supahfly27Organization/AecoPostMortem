...
### The derived tables' DDL is generated by hand, from the design-time model, and versioned by its own hash

`ExcludeFromMigrations` — the same flag that keeps the eight derived tables out of migrations —
also makes `IMigrationsModelDiffer` skip them, so every EF-generated script, including
`GenerateCreateScript()`, omits them too. `DerivedSchema.CreateStatements` therefore builds each
`CREATE TABLE` / `CREATE INDEX` by hand from the model's own metadata (columns, primary key, check
constraints, indexes), and `DerivedSchema.Version` is a SHA-256 over exactly those statements —
computed rather than hand-maintained, so it cannot go stale the way an incremented integer could.
`LocalStore.Open` calls `DerivedSchema.EnsureCurrent` between `Migrate()` and the owner-only file
lock: a version that differs from the one recorded in `store_metadata` means the tables predate the
model, so they are dropped and recreated rather than migrated — the rows go with them, which is
correct, because they are re-derivable from RAW (PRD §3.8).

Every model read inside `DerivedSchema` goes through `context.GetService<IDesignTimeModel>().Model`,
never `context.Model`. `context.Model` is EF Core 10's read-optimized runtime model, and it strips
migrations-era annotations — check constraints are exactly that kind. Reading the runtime model here
would silently generate `CREATE TABLE` statements with no `CHECK` clause: the tables would still get
created, `EnsureCurrent` would still succeed, and the ownership-pairing (`ck_<table>_owner`) and
agent-cost (`ck_agent_cost`) invariants would simply not be enforced, with nothing failing to say so.
`DerivedSchemaTests`' `DerivedTables` test helper reads the model the same way, for the same reason.
