...
| `src/routes/SettingsPage.tsx` | The Settings surface, both parts: Part A renders `SettingsEnvelope` as a `role="group"` figure list — the store path/existence/size, the Copilot source root and whether it was found, and the configured exclusion list, with an honest "Does not exist yet" sentence for a store that doesn't (never a bare `0 bytes`, `web/CLAUDE.md`'s own "an honest empty state" discipline). Part B renders one card per write action (`WriteOperationCard`), each with its own button, a `role="status"` "Running …" sentence while in flight, disabling *both* buttons while either runs, a `role="alert"` failure message that shows the server's own real error (including a distinct "already running" for a `409`), and — on success — the real served result (`IngestResultEnvelope`'s coverage figures / `RebuildResultEnvelope`'s counts), never reduced to a bare "done". Part C adds a third card, the only destructive one: `WriteOperationCard` gained two optional props (`destructive`, rendered as `data-destructive="true"` rather than a second class, and `confirmationField`, rendered between the description and the button), and the card's button stays disabled until `ConfirmationField`'s typed word matches `PurgeConfirmation` exactly — see "A destructive button is armed by typing, not by clicking" below. `PurgeResultSummary` renders the two real outcomes distinctly: what was deleted and how much that reclaimed (naming every deleted path), or a stated "There was no store to purge" — never "Deleted 0 files" |
...
| `src/digest/MethodologyFooter.tsx` | Mockup parity item #9 (`docs/product-superpowers/discovery/2026-08-21-ui-mockup-parity.md`, "Methodology footer"): states what was measured and how the per-finding session strip's positions are sourced. Unlike the mockup's own footer — one fixed set of numbers hand-typed for one frozen date (`~/.copilot/` on 2026-08-16) — every figure here is read straight off the `MastheadEnvelope` this page already fetched, the same "nothing on this page counts anything" discipline `Masthead.tsx` documents for its own figures; no separate fetch, no recomputation. Carries no "not measured, shown only to demonstrate the layout" caveat paragraph — that is the mockup admitting its own placeholder data, and this app's findings are all real. `formatSpan`/number formatting are reimplemented locally (not imported) rather than exported from `Masthead.tsx`, since this story's own scope kept that file untouched. Code review Important (both reviews): an optional `range` prop (`{from, to} | null`, default `null`) adds a second paragraph — "Ranked over N of M sessions, …" — and a clause on the session-strip sentence ("within the applied date range") whenever a filter is active, so this footer's own stated job ("what was measured") stays true instead of contradicting the corpus-wide first paragraph; `null` (no filter) renders neither, byte-for-byte the same as before the prop existed. The repository-filter task changed the session-strip sentence to name the repository (`masthead.repositoryScope.selectedRepository`, already in hand — no new prop) instead of saying "the selected repository", which was unambiguous only while exactly one repository was ever reachable; a `null` selected repository keeps the original wording, since there is no name to give |
...
| `src/digest/RepositorySelector.tsx` | Scenario 3 / PRD Part 8 Q5: shows the selected repository and offers every available one. Unchanged in code by the repository-filter task and still a pure presenter — but no longer a seam: `DigestPage` now re-fetches on the choice it reports, so this control genuinely re-scopes the digest. See "The repository selector re-scopes the digest" below, including why the name it shows is the *requested* repository rather than the served one |
...
### The repository selector re-scopes the digest (superseded the seam below)

**This section's original decision has been reversed; the text below is kept as the record of what
was there and why it had to change.** `DigestPage` now holds the selection in state, passes it to
`useDigest` as a third scalar, and the server re-scopes the whole analysis against it
(`ApiHost.RepositoryParameter`, `Api/CLAUDE.md`'s matching entry) — the identical "a new request,
not a filter over what is already loaded" shape the date range and the Rules Inventory's version
switch both already use. `pendingRepository` is renamed `selectedRepository` to stop the name
implying a choice that never goes anywhere.

Two details that are load-bearing, both caught by tests written before the code:

- **The selector shows the *requested* repository, not the served one.** A re-fetch deliberately
  keeps the previous digest mounted (`isRefetching`, below), so `displayedScope` overlays the
  requested selection onto the served scope — reading straight off `scope` makes the `<select>` snap
  back to the previous repository for the whole request and read as having rejected the click.
  `DateRangeFilter` already shows its typed values rather than the served ones for exactly this
  reason. The overlay expression is the same one the seam used; only what it feeds changed.
- **Selecting resets the pager to page 1**, for the same reason `applyRange` does: the ranked list is
  re-scoped server-side, so the previous repository's page position means nothing against it.

`RepositorySelector.tsx` itself needed no code change — it already reported the choice through
`onSelect`. `digest/MethodologyFooter.tsx` did: its session-strip sentence said "the N sessions in
the selected repository", unambiguous only while there was exactly one repository the digest could
possibly be showing. It now names the repository, read off `masthead.repositoryScope` it already
receives (no new prop), falling back to the original wording when `selectedRepository` is genuinely
`null` — a store where no session records a repository at all.

Deliberately still open, not oversights: `/rules` and `/monitor` remain pinned to the server's
most-sessions default with no selector of their own, and the selection is not reflected in the URL
(no deep-linkable filtered digest) — the same round trip the date filter also deferred.

### The repository selector is a seam, not a working cross-repository switch (superseded — see above)
...
### A destructive button is armed by typing, not by clicking

The purge card destroys the operator's whole store, so it cannot be one click away the way ingest and
rebuild are. Three mechanisms were available: a second confirming click, a native `confirm()`
dialogue, or a typed word. The typed word won on two counts. A second click is muscle memory — the
failure it guards against (clicking the wrong control) is exactly the one it fails to stop. A native
`confirm()` blocks the whole renderer, which this project's own real-browser verification passes
cannot drive at all (a modal dialogue freezes every subsequent automation command), so a gate built
that way would be a gate nothing could ever check end to end. Typing `purge` requires reading, and
leaves a real DOM state a test and a browser check can both assert.

`SettingsPage` owns the typed value, not `WriteOperationCard`, for one reason worth stating: it has
to be *cleared* after a run — whatever the outcome. A refused or failed purge that left the field
filled would sit one click away from retrying a destructive action the operator has not re-confirmed,
so `runPurge` clears it before branching on success, unlike `afterWrite`'s own success-only gating.
The field is also disabled while any write is running, so the card cannot be re-armed mid-operation.

The typed word is only the operator's half of the confirmation. The machine half is the
`ConfirmationHeader` `postPurge` sends, which the server independently requires
(`Api/CLAUDE.md`'s "A destructive route needs a gate that proves intent, not only provenance") — the
button being disabled is a UI affordance, and a client that bypassed it entirely would still be
refused. `ConfirmationField` uses explicit `htmlFor`/`id` rather than an implicit wrapping label, the
same association `DateRangeFilter` had to be corrected to after a real-browser accessibility check
found jsdom and Chrome disagreeing about implicit labels; confirmed in Chrome's own accessibility
tree here rather than assumed ("Type purge to confirm").
