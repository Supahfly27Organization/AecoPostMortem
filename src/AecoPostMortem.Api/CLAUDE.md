...
`ruleShapeMatches` and calls the new `ParamCarryingCallLookup.BuildAll(scopedToolCalls, scopedAgents,
scopedRawEvents)` — the same three scoped reads `ToolInvocationShapeLookup.BuildAll` already takes,
just building a different corpus from them — as `GetDigest`'s tenth and final piece-3 check
orchestrator. Unlike every other `RuleAdherenceToolChoice` finding here, `AlwaysPassParamFinding.Run`
takes no `ToolCall` parameter at all: `ParamCarryingCall` already carries `SessionId` (it was built
specifically for this check, not shared the way `ToolInvocationShape` is), so there is no separate
entity read needed for session attribution.
